...
def createcircles():
	...
	for i in range(1000):
		...
		draw.ellipse((t, u, x, y), fill = randcolor(),outline =randcolor())
	...
	for i in range(1000,2000):
		...
		draw = ImageDraw.Draw(image)
		draw.ellipse((t, u,x, y), fill = c,outline ='black')
		image.save('./data_geometricalshapes/circles//'+str(i)+'tt.jpeg')
		#remaining circles
	for i in range(2000,5000):
	    ...
	    draw = ImageDraw.Draw(image)
	    draw.ellipse((t,t,x,x), fill = w,outline =z)
	    image.save('./data_geometricalshapes/circles//'+str(i)+'tt.jpeg')
...
def createtriangles():
	for i in range(5000,10000):
	    ...
	    v=randcolor()
	    v_=randcolor()
	    draw.polygon([(0, 0), (0, 200), (200, 200), (200, 0)], fill = v)
	    if i<9000:
	        draw.polygon([(x1,y1), (x2, y2), (x3,y3)], fill = v_) 
	        im.save('./data_geometricalshapes/triangles//'+str(i)+'tt.jpeg')
	    else:
	        draw.polygon([(x1,y1), (x2, y2), (x3,y3)], outline='black',fill = v) 
	        im.save('./data_geometricalshapes/triangles//'+str(i)+'tt.jpeg')
...
def createrectangles():
	for i in range(10000,15000):
	    ...
	    v_=randcolor()
	    draw.polygon([(0, 0), (0, 200), (200, 200), (200, 0)], fill = v)
	    draw.polygon([(x1,y1), (x2, y2), (x3,y3),(x4,y4)], fill = v_)
	    if i<14000:
	        draw.polygon([(x1,y1), (x2, y2), (x3,y4),(x4,y3)], fill = v_)
	        im.save('./data_geometricalshapes/rectangles//'+str(i)+'tt.jpeg')
	    else:
	        draw.polygon([(x1,y1), (x2, y2), (x3,y4),(x4,y3)], outline='black',fill = v) 
	        im.save('./data_geometricalshapes/rectangles//'+str(i)+'tt.jpeg')
...
def createpentagons():
	for i in range(15000,20000):
	    ...
	    if i<19000:
	        draw.polygon([tuple(pg[0]),tuple(pg[1]),tuple(pg[2]),tuple(pg[3]),tuple(pg[4])],fill =v_)
	        im.save('./data_geometricalshapes/pentagons//'+str(i)+'tt.jpeg')
	    else:
	        draw.polygon([tuple(pg[0]),tuple(pg[1]),tuple(pg[2]),tuple(pg[3]),tuple(pg[4])], outline='black',fill =v)
	        im.save('./data_geometricalshapes/pentagons//'+str(i)+'tt.jpeg')
